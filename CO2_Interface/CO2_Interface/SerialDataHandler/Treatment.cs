    ...
    internal static partial class Reception
    {
        ...
        private static void CSCheck(FromSensor.Base obj)
        {
            ...
            if (checksum != obj.Checksum)
            {
                ...
                Data.Errors err = new Data.Errors(2, "Transimission Error !" + checksum + " - " + obj.Checksum);
        ...
        internal static void LoadConfig()
        {
            ...
            try
            {
                ...
                else MessageBox.Show("Corrupted"); errorDetected = "Le fichier est corrompu";
                //else ConfigFile_Status.Text = "Corrupted";
            }
            catch (FileNotFoundException)
            {
                MessageBox.Show("Not found");
                errorDetected = "Le fichier n'a pas été trouvé";
                ///ConfigFile_Status.Text = "Not found";
            }
            catch (IOException)
            {
                MessageBox.Show("Not loaded");
                errorDetected = "Le fichier n'a pas pu être chargé";
            ...
            {
                Data.Errors err = new Data.Errors(3, errorDetected);
                SerialDataHandler.Reception.LoadErrors(err, Data.Tables.DataErrors);
            }
                /************* CODE EXAMEN: FIN **************/
        ...
        internal static void LoadErrors(Data.Errors newError, DataTable dt)
        {
            dt.Rows.Clear();

            bool errFound = false;
            foreach (Data.Errors err in Data.Collections.ErrorList)
            {
                //Si on ne l ajoute pas 
                if (err.Ref == newError.Ref)
                {
                    errFound = true;
                    break;
                }
            ...
            if (errFound != true)//Sinn on l'ajoute dans la liste
            {
                //erreur lors de l'Insertion
                //DBAccess_Provider.Adapter.InsertErrors(newError.Ref, newError.Type, newError.Date, newError.Description);
                Data.Collections.ErrorList.Add(newError);
            } else
            {
                newError = null; //Supression de l'erreur
            }
        ...
        internal static void LoadErrorsToDataTable(DataTable dt, DataGridView dg, ErrorsTable errorsTablePage)
        {
            dt.Rows.Clear();

            int toDisplayType = 0;

            if (errorsTablePage.rAll.Checked)
            {
                toDisplayType = 0;
            }
            else if (errorsTablePage.rType1.Checked)
            {
                toDisplayType = 1;
            }
            else if (errorsTablePage.rType2.Checked)
            {
                toDisplayType = 2;
            }
            else if (errorsTablePage.rType3.Checked)
            {
                toDisplayType = 3;
            }
            else if (errorsTablePage.rType4.Checked)
            {
                toDisplayType = 4;
            }
            else if (errorsTablePage.rType5.Checked)
            {
                toDisplayType = 5;
            }
            ...
            foreach (Data.Errors err in Data.Collections.ErrorList)
            {
                if (toDisplayType!=0)
                {
                    if (err.Type == (byte)toDisplayType)
                    {
                        dt.Rows.Add(new object[] { err.Ref, err.Type, err.Date, err.Description });
                    }
                } else
                {
                    dt.Rows.Add(new object[] { err.Ref, err.Type, err.Date, err.Description });
